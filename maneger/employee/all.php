    ...
    <body>
        ...
        <div class="container">          
            ...
            <div class="content">
                ...
                <?php
                $db = DB();
                $sql = "SELECT * FROM \"管理員\" ORDER BY \"管理員編號\"";
                $result = $db->query($sql);
//        echo '<table  border="1">';
...
//        echo '</table>';
                ?>
                <P> 搜尋管理員：</p><input type="search" class="light-table-filter" data-table="order-table" placeholder="請輸入關鍵字">
                ...
                <table id="table-3" class="order-table"   >
                    ...
                    <tbody>
                        ...
                        while ($row = $result->fetch(PDO::FETCH_OBJ)) {
                            //PDO::FETCH_OBJ 指定取出資料的型態
                            echo '<tr>';
                            echo '<td>' . $row->管理員編號 . "</td>"
                            . "<td>" . $row->管理員姓名 . "</td>"
                            . "<td>" . $row->帳號 . "</td>"
                            . "<td>" . $row->密碼 . "</td>"
                            . "<td> <button type=\"button\" onclick='location.href=\"change.php?id=" . $row->管理員編號 . "\"'>更新</button></td>"
                            . "<td> <button type=\"button\" onclick='location.href=\"delete.php?id=" . $row->管理員編號 . "\"'>刪除</button></td>";
